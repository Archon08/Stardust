...
protected:
	String skillName = "polearmleghit2";		// Skill Name
	String skillNameDisplay = "Improved Polearm Leg Hit";		// Skill Display Name for output message
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
			...
			if (res == SUCCESS) {
				...
				if (!creature->checkCooldownRecovery(skillName)){
					...
					buff->setSpeedMultiplierMod(0.60f);
					buff->setAccelerationMultiplierMod(0.60f);
					targetCreature->setSnaredState(7);
