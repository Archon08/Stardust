...
protected:
	String skillName = "polearmleghit3";		// Skill Name
	String skillNameDisplay = "Advanced Polearm Leg Hit";		// Skill Display Name for output message
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
			...
			if (res == SUCCESS) {
				...
				if (!creature->checkCooldownRecovery(skillName)){
					...
					targetCreature->setSnaredState(9);
